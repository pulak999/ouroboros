...
def main() -> None:
    ...
    probe: dict[int, dict] = {}
    # The probe file opens with a _meta record naming the driver it was measured
    # against. Read it rather than hard-coding a version into the report — that
    # is exactly how this doc came to claim 555.42.02 while the box ran 610.
    driver_ver = "unknown"
    for line in args.probe.read_text(encoding="utf-8").splitlines():
        ...
        if "cmd" not in r:
            meta = r.get("_meta") or {}
            m = re.search(r"\b(\d+\.\d+\.\d+)\b", str(meta.get("driver", "")))
            if m:
    ...
    write_markdown(rows, args.out_md, driver_ver)
...
def write_markdown(rows: list[dict], out: Path, driver_ver: str = "unknown") -> None:
    L: list[str] = []
    A = L.append
    A(f"# MIG command classification — driver {driver_ver} on hulk (TITAN RTX)")
    A("")
    A("Generated by `cuda-ioctl-map/tools/effectmap/classify_mig.py`. Do not")
    A("edit by hand; re-run the tool.")
    A("")

    n = {t: sum(1 for r in rows if r["tier"] == t)
         for t in ("config", "runtime", "internal", "unrelated")}
    present = [r for r in rows if r["driver_present"]]
    absent = [r for r in rows if r["driver_present"] is False]
    observed = [r for r in rows if r["observed_in_traces"]]
    internal_present = [r for r in rows
                        if r["tier"] == "internal" and r["driver_present"]
                        and r["driver_params_size"] not in (None, -1)]

    A("## Answer in short")
    A("")
    A(f"Of {len(rows)} commands that mention MIG, partitions, SMC or migration:")
    A("")
    A(f"- **{n['config']}** are **MIG configuration**, reachable by ioctl. This is")
    A("  the set that reshapes the GPU.")
    A(f"- **{n['runtime']}** are **MIG-aware runtime queries**, reachable by ioctl.")
    A(f"- **{n['internal']}** are **RM-to-GSP internal**. These are *not* ioctls at")
    A("  all. No userspace tracer will ever see them.")
    A(f"- **{n['unrelated']}** only matched a keyword and are **not MIG** — mostly VM")
    A("  live migration and TPC partition mode.")
    A("")
    A(f"{len(present)} of these commands are compiled into the {driver_ver} driver and")
    A(f"{len(absent)} are not, measured against the loaded binary. Because the size")
    A("check runs before the driver looks at the GPU, this is a property of the")
    A("driver build and holds even though a TITAN RTX cannot do MIG.")
    A("")
    if observed:
        A("**MIG commands that a plain `cuInit` already issues on this non-MIG box:**")
        A("")
        for r in observed:
            A(f"- `{r['cmd_hex']}` {r['name']} — {r['observed_count']} calls in "
              f"{', '.join(r['observed_programs'])}")
        A("")
        A("So the MIG query path is not conditional on MIG hardware. CUDA asks")
        A("about partitions on every GPU, and the driver answers.")
        A("")
    A("### Next step — the RPCs to reverse engineer")
    A("")
    A("Tier 3 is the answer to \"which RPCs do we go after next\". They are")
    A("`present` in this driver, so their sizes are recoverable here even without")
    A("A100 hardware, but they are unreachable through `NV_ESC_RM_CONTROL`, so")
    A("Experiment B (the GSP RPC tracer) is the only way to observe them firing.")
    A(f"{len(internal_present)} of them already have a driver-verified paramsSize")
    A("in the table below, which is the input a decoder needs.")
    A("")
    A("The two the plan calls out by name are both present here:")
    A("")
    for want in ("NV2080_CTRL_CMD_INTERNAL_MIGMGR_SET_GPU_INSTANCES",
                 "NV2080_CTRL_CMD_INTERNAL_MEMSYS_SET_PARTITIONABLE_MEM"):
        hit = next((r for r in rows if r["name"] == want), None)
        if hit:
            A(f"- `{hit['cmd_hex']}` {want} — {_present_cell(hit)}, "
              f"paramsSize {hit['driver_params_size']}")
    A("")
    A("## How to read this")
    A("")
    ...
    A(f"- **driver** — measured against the loaded {driver_ver} binary with")
